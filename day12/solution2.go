...
func main() {
	...
	possibleSource := make([]int, 0)

	for i := 0; i < nRows; i++ {
		for j := 0; j < nCols; j++ {
			if heightmap[i][j] == 0 {
				possibleSource = append(possibleSource, j+i*nCols)
			}
		}
	}
	...
	for _, source := range possibleSource {
	...
	sort.Ints(allSteps)
	fmt.Println("Num. of fewest Steps:", allSteps[0])
}
